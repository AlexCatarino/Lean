...
namespace QuantConnect.ToolBox.BovespaDownloader
{
    class OldProgram
    {
        ...
        {
            args = new string[] { "PETR4", "Daily", "20150103", "20160104" };
            ...
            try
            {
                ...
                var downloader = new BovespaFactorDownloader();

                //downloader.WriteInstrumentsFile();
                ...
                var data = new List<BaseData>();// downloader.Get(symbol, securityType, resolution, startDate, endDate);

                var argh = data.ToList();
            ...
            foreach (var zipFile in zipFiles)
            {
                ...
                ((await ReadAsyncZipFile(zipFile, _config.InstrumentList)).FindAll(d => !ValidateSymbol(d))
                    .Select(d =>
                    {
                        ...
                        tick.Symbol = null;// strarr[1].Trim();
        ...
        private static async Task DailyFilesConverter()
        {
            ...
            foreach (var zipfile in zipFiles)
            {
                ...
                tradeBars.AddRange((await ReadAsyncZipFile(zipfile)).FindAll(d => !Filter(d))
                    .Select(d =>
                    {
                        return new TradeBar(d.Substring(2).ToDateTime(), null,//d.Substring(12, 12).Trim(),
        ...
        private static async Task<string> DownloadAsync(string str)
        {
            ...
            //    using (var response = client.DownloadDataAsync(new Uri(str)))
